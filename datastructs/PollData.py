...
class PollData:
    ...
    def data_summary(self):
        ...
        summary = {
            ...
            "trial_info": dataclasses.asdict(self.trial_info) if self.trial_info else None,
    ...
    def save_to_file(self):
        ...
        self._lcr_measurements.to_csv(f"{root_dir}/raw/{filename}/lcr_measurements.csv", index=False)
        self._adc_measurements.to_csv(f"{root_dir}/raw/{filename}/adc_measurements.csv", index=False)
        self._poi_measurements.to_csv(f"{root_dir}/raw/{filename}/poi_measurements.csv", index=False)
    ...
    def _prep_file_prefix(self):
        ...
        return test_id, f"PL_{test_id:04d}_{self.poll_parameters.trial_info.resin_type}_{now.strftime("%H%M")}"
...
class PollParameters:
    ...
    trial_info: TrialInfo = field(default_factory=lambda: TrialInfo(trial_name="Default Trial", resin_type="Unknown"))
    push_force: float = 70
    push_repeats: int = 2
