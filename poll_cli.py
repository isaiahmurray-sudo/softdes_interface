...
def get_connection_parameters():
    ...
    debug = (
        input("Use debug mode (simulated hardware)? (y/n) [default: y]: ")
        .strip()
        .lower()
    )
    ...
    return {"debug": debug, "leash_ip": leash_ip, "lcr_port": lcr_port}
...
def get_poll_parameters():
    ...
    while True:
        ...
        image_str = (
            input("Enter exposure image (V1/V2) [default: V2]: ").strip().upper()
        )
        image = ExposureImage.V2 if image_str == "V2" else ExposureImage.V1

        exposure_params.append(
            ExposureParameter(duration=duration, intensity=intensity, image=image)
        )

        more = (
            input("Add another exposure segment? (y/n) [default: n]: ").strip().lower()
        )
        if more != "y":
            break
    ...
    if not exposure_params:
        ...
        exposure_params = [
            ExposureParameter(duration=5, intensity=0, image=ExposureImage.V2)
        ]
...
def run_poll(params, conn_params):
    ...
    try:
        leash = (
            ...
            else LeashControllerDebug()
        )
        adc = (
            ADCControllerHardware()
            if not conn_params["debug"]
            else ADCControllerDebug()
        )
        lcr = (
            LCRControllerHardware()
            if not conn_params["debug"]
            else LCRControllerDebug()
        )
